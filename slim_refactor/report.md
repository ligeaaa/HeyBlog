...
- scope: backend + crawler + bounded persistence repository convergence under strict mode
- objective: reduce duplicated backend/persistence error, counting, pagination, and crawler filter helper logic
...
- simplified:
  ...
  - repository row counting into `_count_selectable_rows()`.
  - repository paginated query execution into `_execute_paginated_query()` and `_resolve_page_window()`.
...
- before:
  ...
  - repository catalog and blog-labeling queries each computed `total_items`, `total_pages`, `effective_page`, and `offset` inline.
  - repository stats/reset/refilter/dedup code repeated raw `select(func.count()).select_from(...)` count expressions.
- after:
  ...
  - repository count-heavy paths call one count helper, and paginated query paths call one shared pagination executor.
- why this is smaller and clearer:
  ...
  - repository query methods now focus on query semantics rather than repeated page math and count plumbing.
...
- reduced state forks:
  ...
  - repository pagination semantics now have one effective-page implementation instead of two drifting copies.
...
- reduced coupling paths:
  ...
  - repository maintenance/stats paths now share one count helper instead of each carrying a private count expression.
...
| `persistence_api/repository.py` | A5, A6 | unify row counting and pagination execution semantics across repository flows | `./.venv/bin/pytest tests/test_repository.py`; `./.venv/bin/pytest`; `python3 -m compileall persistence_api` |
...
- key outputs:
  ...
  - `32 passed in 6.71s`
  - `154 passed, 10 warnings in 4.58s`
  - compileall completed successfully for `persistence_api`
- pass or fail:
  - pass
...
- residual risks:
  ...
  - full `pytest` still emits pre-existing `ResourceWarning` noise around sqlite handles in `tests/test_pipeline.py`.
- deferred items:
