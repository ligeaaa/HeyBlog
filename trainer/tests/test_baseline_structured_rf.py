...
def test_structured_rf_baseline_can_fit_and_predict() -> None:
    samples = [
    ...
    model = train_structured_rf_baseline(samples, structured_rf_model_config())

    probabilities = model.predict_proba(samples)

    assert model.model_name == "structured_rf"
    assert len(probabilities) == 6
    assert all(0.0 <= probability <= 1.0 for probability in probabilities)
    assert "feature_importances" in model.feature_summary()
